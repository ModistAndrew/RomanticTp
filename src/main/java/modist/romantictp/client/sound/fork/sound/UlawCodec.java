...
public final class UlawCodec extends FormatConversionProvider {
    ...
    public Encoding[] getTargetEncodings(AudioFormat sourceFormat){
        if( Encoding.PCM_SIGNED.equals(sourceFormat.getEncoding()) ) {
        ...
        } else if (Encoding.ULAW.equals(sourceFormat.getEncoding())) {
            if (sourceFormat.getSampleSizeInBits() == 8) {
                Encoding[] enc = new Encoding[1];
                enc[0] = Encoding.PCM_SIGNED;
                return enc;
            } else {
                return new Encoding[0];
            }
    ...
    public AudioFormat[] getTargetFormats(Encoding targetEncoding, AudioFormat sourceFormat){
        ...
        if( (Encoding.PCM_SIGNED.equals(targetEncoding)
             && Encoding.ULAW.equals(sourceFormat.getEncoding()))
            ||
            (Encoding.ULAW.equals(targetEncoding)
             && Encoding.PCM_SIGNED.equals(sourceFormat.getEncoding()))) {
    ...
    public AudioInputStream getAudioInputStream(Encoding targetEncoding, AudioInputStream sourceStream){
        ...
        if (Encoding.ULAW.equals(sourceEncoding) &&
            Encoding.PCM_SIGNED.equals(targetEncoding) ) {
        ...
        } else if (Encoding.PCM_SIGNED.equals(sourceEncoding) &&
                   Encoding.ULAW.equals(targetEncoding)) {
            targetFormat = new AudioFormat( targetEncoding,
                                            sourceFormat.getSampleRate(),
                                            8,
    ...
    private AudioFormat[] getOutputFormats(AudioFormat inputFormat) {
        ...
        if ((inputFormat.getSampleSizeInBits() == 16)
            && Encoding.PCM_SIGNED.equals(inputFormat.getEncoding())) {
            format = new AudioFormat(Encoding.ULAW,
        ...
        if (inputFormat.getSampleSizeInBits() == 8
                && Encoding.ULAW.equals(inputFormat.getEncoding())) {
    ...
    private final class UlawCodecStream extends AudioInputStream {
        ...
        UlawCodecStream(AudioInputStream stream, AudioFormat outputFormat) {
            ...
            if (Encoding.ULAW.equals(inputFormat.getEncoding())) {
                encode = false;
