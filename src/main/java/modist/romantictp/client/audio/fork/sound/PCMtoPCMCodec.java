...
package modist.romantictp.client.audio.fork.sound;
...
public final class PCMtoPCMCodec extends FormatConversionProvider {
    ...
    public Encoding[] getTargetEncodings(AudioFormat sourceFormat) {
        ...
        Encoding encoding = sourceFormat.getEncoding();
        if (sampleSize == 8) {
    ...
    private AudioFormat[] getOutputFormats(AudioFormat inputFormat) {
        ...
        if ( sampleSize==8 ) {
            if ( Encoding.PCM_SIGNED.equals(inputFormat.getEncoding()) ) {

                format = new AudioFormat(Encoding.PCM_UNSIGNED,
            ...
            if ( Encoding.PCM_UNSIGNED.equals(inputFormat.getEncoding()) ) {
            ...
            if ( Encoding.PCM_SIGNED.equals(inputFormat.getEncoding()) && isBigEndian ) {
            ...
            if ( Encoding.PCM_UNSIGNED.equals(inputFormat.getEncoding()) && isBigEndian ) {
    ...
    class PCMtoPCMCodecStream extends AudioInputStream {
        ...
        PCMtoPCMCodecStream(AudioInputStream stream, AudioFormat outputFormat) {
            ...
            if( sampleSizeInBits==8 ) {
                if( Encoding.PCM_UNSIGNED.equals(inputEncoding) &&
                    Encoding.PCM_SIGNED.equals(outputEncoding) ) {
                    conversionType = PCM_SWITCH_SIGNED_8BIT;
                } else if( Encoding.PCM_SIGNED.equals(inputEncoding) &&
                           Encoding.PCM_UNSIGNED.equals(outputEncoding) ) {
                    conversionType = PCM_SWITCH_SIGNED_8BIT;
                }
            } else {
                ...
                } else if (Encoding.PCM_UNSIGNED.equals(inputEncoding) && !inputIsBigEndian &&
                            Encoding.PCM_SIGNED.equals(outputEncoding) && outputIsBigEndian) {

                    conversionType = PCM_UNSIGNED_LE2SIGNED_BE;
                } else if (Encoding.PCM_SIGNED.equals(inputEncoding) && !inputIsBigEndian &&
                           Encoding.PCM_UNSIGNED.equals(outputEncoding) && outputIsBigEndian) {
